...
namespace ORB_SLAM2 {
    ...
    void Tracking::Track() {
        ...
        if (mState == NOT_INITIALIZED) {
            ...
            // STS: update tracked points
            this->UpdateTrackedMapPointsNumber();
    ...
    void Tracking::StereoInitialization() {
        if (mCurrentFrame.N > 500) {
            ...
            cout << "New map " << mpMap->mnId << " created with " << mpMap->MapPointsInMap() << " points" << endl;
...
//        mpMap->mvpKeyFrameOrigins.push_back(pKFini);
    ...
    bool Tracking::NeedNewKeyFrame() {
        ...
        const float ratioMap = float(nMap) / max(1, nTotal);
    ...
    void Tracking::Reset() {
        ...
        if (!mpViewer->isFinished()) {

            cout << "Resetting Viewer" << endl;
...
//        mpLoopClosing->RequestReset();
        ...
        KeyFrame::nNextIds[mpMap->mnId] = 0;
//        KeyFrame::nNextId = 0;
        Frame::nNextId = 0;
//        MapPoint::nNextId = 0;
        // TODO<halcao>: why not MapPoint::nNextId ?
    ...
    void Tracking::UpdateTrackedMapPointsNumber() {
        unique_lock<mutex> lock(mMutexTrackedMapPoints);

        int nTracked = 0;
        auto outliers = mCurrentFrame.mvbOutlier;
        auto points = mCurrentFrame.mvpMapPoints;
        for (size_t i = 0; i < points.size(); i++) {
            MapPoint *pMP = points[i];
            if (!pMP) continue;

            if (!outliers[i] && pMP->Observations() > 0) {
                nTracked++;
            }
        }

        this->mnTrackedMapPoints = nTracked;
    }

    int Tracking::GetTrackedMapPointsNumber() {
        unique_lock<mutex> lock(mMutexTrackedMapPoints);
        return this->mnTrackedMapPoints;
    }

    void Tracking::UpdateAverageVelocity() {
        if (mState == TRACKING_LOST || mCurrentFrame.mTcw.empty()) return;

        // TODO(halcao): make it configurable
        const auto N = 10;

        auto Tcw = mCurrentFrame.mTcw;
        auto Rwc = Tcw.rowRange(0, 3).colRange(0, 3).t();
        cv::Mat twc = -Rwc * Tcw.rowRange(0, 3).col(3);

        // get world coordinate of current frame
        this->mvTrace.push_back(twc);

        if (this->mvTrace.size() > N) {
            // if size is larger than N, remove the oldest one
            this->mvTrace.erase(this->mvTrace.begin());
        }

        if (this->mvTrace.size() == 1) {
            // init the average velocity
            this->mAvgVelocity = 0;
            this->mbVelocityBurst = false;
            return;
        }
        ...
        // last of trace
        double last = 0;
        for (size_t i = 0; i < this->mvTrace.size() - 1; i++) {
            const auto distance = cv::norm(this->mvTrace[i + 1] - this->mvTrace[i]);
            sum += distance;
//            info("distance {}: {}", i, distance);
            if (i == this->mvTrace.size() - 2) {
                last = distance;
            }
        }

//        info("last distance: {}, last average distance: {}", last, this->mAvgVelocity);

        if (this->mAvgVelocity > 0) {
            // update mbVelocityBurst
            this->mbVelocityBurst = last > (this->mAvgVelocity * 1.2);
        } else {
            this->mbVelocityBurst = false;
        }
        debug("velocity burst: {}", this->mbVelocityBurst);

        this->mAvgVelocity = sum / double(this->mvTrace.size() - 1);
//        info("new average distance: {}", this->mAvgVelocity);
    }
